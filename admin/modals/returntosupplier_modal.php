...
//yearly reset to 1
$id = $db->query("select count(idreturns)+1 as cnt from tbl_returntosupplier where year(returneddate) = year(now()) limit 1;")->fetch_assoc();
$num1 = $id['cnt'];

//oldies loop but goodies to search id if exists
back:
$rno = 'MTs'.$_SESSION["area"].$mos.$year.'-'.$num1;
$id = $db->query("select returnedno from tbl_returntosupplier where returnedno like '$rno' limit 1;")->fetch_assoc();
if($id>0) {
  $num1 += 1;
  goto back;
}
...
<div class="modal fade" id="modal-addreturn">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form class="form-horizontal" method="POST" action="datahelpers/returntosupplier_helper.php"> 
        <div class="modal-body">
          ...
          <div class="form-group row">
            ...
            <div class="col">              
              <select class="form-control form-control-sm select2" id="suppliercode" name="suppliercode" required>
    ...
    <div class="modal-content">
      ...
      <form class="form-horizontal" method="POST" action="datahelpers/returntosupplier_helper.php">
        <div class="modal-body">
          ...
          <div class="form-group row">
            <label class="col-sm-3 col-form-label">Coltrol No.</label>
            <div class="col">
          ...
          <div class="form-group row">
            <label class="col-sm-3 col-form-label">Return To</label>
            <div class="col">              
              <select class="form-control form-control-sm select2" id="esuppliercode" name="esuppliercode" required>
                <?php foreach($suppliers as $row): ?>
                  <option value="<?= $row['suppliercode']; ?>"><?= $row['suppliername']; ?></option>
                <?php endforeach; ?>
              </select>
...
<script type="text/javascript">
  ...
  function returnFrom() {
    /*var selectedid = $('#returnfrom').val();
    if(selectedid == 0)
      $('#label-reference').html("Reference MCT");
    else
      $('#label-reference').html("Reference MR");*/
  }
